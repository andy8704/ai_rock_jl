...
// Size (2 bytes): 该属性条目的总字节数。
// Flags/Permissions (2 bytes): 定义属性的权限和特性（如 READ, WRITE, NOTIFY, DYNAMIC 等）。
// Handle (2 bytes): 该属性在 GATT 服务器中的唯一句柄。
// UUID (2 or 16 bytes): 属性的类型 UUID。16 位 UUID 用于蓝牙 SIG 定义的标准类型，128 位 UUID 用于自定义类型。
// Value/Data: 对于某些属性类型（如 Characteristic Declaration），这部分可能包含进一步的定义信息，而不是实际的值。
